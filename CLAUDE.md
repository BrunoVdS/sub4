...
**The suite is the gate, and it is fast.** 1,900 tests in 183 suites at patch 440,
well under a second of test time. The run prints the current total; this figure is here for the order of
...
declared EMPTY DIRECTORY as copied with **no hash**, and the writer tried to
hash it. **Every container in the suite held only files**, which is why every
control passed. §12.85.7's family: a fixture that never varies never asks.
§12.202.
...
**AND 441 CAN CLEAR THE LEFTOVER, WITH FOUR REFUSALS.** Preview → confirm →
remove → verify → receipt, and `remove` RE-EVALUATES every refusal rather than
trusting the preview it is handed. It refuses while anything is hidden, unless
the live counterpart exists **and reads**, unless the path is on an allow-list
built from `LegacyFileTest.names` (never a glob), and unless a complete snapshot
still on disk holds a hashed copy of that counterpart. The receipt is written
into the folder it cleaned and needed its own status — as `unrecognised` it
would have blocked every later removal. §12.196.
...
**AND RUN IT ONCE ON A FRESHLY ERASED SIMULATOR — 440.**
`SkipStandingTests.recordingASkipRoundTrips` passed for seventy patches only
because an earlier run had left an `account` row in the simulator's container:
it used `Matcher.shared`, which commits to the app's REAL database, and
`match_decision.accountID` references `account`. Erase, run, and the refusal is
correct while the test is not. **That run had never been done.** The suite is
green on a clean container as of 440 — `xcrun simctl erase <udid>` before
`./scripts/test.sh`, whenever a suite starts touching the app's own stores.
§12.195.
...
8. **B5's GROUNDWORK IS WRITTEN — `docs/D7-B5-GROUNDWORK.md`, and it is
   ...
   **428 IS THE MACHINERY: ELEVEN FAMILIES DECLARED, NINE FED**, and that gap
   is the slice. ONE bootstrap field for two families — the read fails as one
   thing (`fieldCount`'s rule, `authored`'s precedent) — and TWO `Family` cases,
